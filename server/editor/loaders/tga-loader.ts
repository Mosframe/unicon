...
export class TGALoader {
	...
	parse ( buffer ) {
		...
		this.use_rle = use_rle;
		this.use_pal = use_pal;
		...
		if( context ) {
			...
			var rgbaData = this.getTgaRGBA( imageData.data, header.width, header.height, result.pixel_data, result.palettes );
	...
	getTgaRGBA( data, width, height, image, palette ) {
		...
		switch ( ( this.header.flags & TGA_ORIGIN_MASK ) >> TGA_ORIGIN_SHIFT ) {
			default:
			case TGA_ORIGIN_UL:
				x_start = 0;
				x_step = 1;
